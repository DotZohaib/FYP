...
def train_face_recognizer(models):
    """Train the face recognizer with the friend images"""
    ...
    for friend_key, friend_info in friend_profiles.items():
        ...
        # Create dummy face for testing if image doesn't exist
        if not os.path.exists(img_path):
            # Ensure directory exists
            os.makedirs(os.path.dirname(img_path), exist_ok=True)

            dummy_face = np.ones((100, 100), dtype=np.uint8) * 200
            cv2.imwrite(img_path, dummy_face)
            st.warning(f"Created dummy face image for {friend_info['name']} at {img_path}")
        ...
        try:
            ...
            if img is not None:
                ...
                for (x, y, w, h) in detected_faces:
                    ...
                    # Resize face to ensure consistent size
    ...
    if faces and labels:
        try:
            models['face_recognizer'].train(faces, np.array(labels))
...
def recognize_friend(img, face_coords, gray_img, models):
    ...
    try:
        # Resize the face to a standard size that the model expects
        face_roi = cv2.resize(face_roi, (100, 100))

        # Predict the label
        label, confidence = models['face_recognizer'].predict(face_roi)

        # Confidence threshold - lower values are more confident
        # For LBPH, adjust this threshold
